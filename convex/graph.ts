...
export const upsertConcept = mutation({
  ...
  handler: async (ctx, args) => {
    const normalized = args.name.toLowerCase().trim();
    const domain = (args.domain ?? "general").toLowerCase().trim();
...
export const getConceptsForDomain = query({
  ...
  handler: async (ctx, args) => {
    ...
    const [concepts, registeredDomains] = await Promise.all([
      ctx.db.query("concepts").collect(),
      ctx.db.query("conceptDomains").collect(),
    ]);
    const sectorMappingByDomain = new Map(
      registeredDomains.map((item) => [item.name, item.sectorMapping]),
    );
    const results = concepts.filter((concept) => {
      const domain = concept.domain.toLowerCase();
      const mappedSector =
        sectorMappingByDomain.get(domain) ?? inferDisplaySectorFromDomain(domain);
      return mappedSector === sector;
    });
