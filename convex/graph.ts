...
async function scheduleConceptClassification(
...
export const getEdgesTo = query({
  ...
  handler: async (ctx, args) => {
    const edgeQuery = ctx.db
      .query("edges")
      .withIndex("by_to", (q) =>
        q.eq("toType", args.toType).eq("toId", args.toId),
      );
...
export const searchSourcesByConcept = query({
  args: {
  ...
  handler: async (ctx, args) => {
    const normalized = args.conceptName.toLowerCase().trim();
    const limit = args.limit ?? 20;
    const edges = await ctx.db
      .query("edges")
      .withIndex("by_to", (q) =>
        q.eq("toType", "concept").eq("toId", normalized),
      )
      .filter((q) => q.eq(q.field("fromType"), "source"))
      .take(limit);
...
export const linkExtractionConcepts = internalAction({
  ...
  handler: async (
    ...
    await scheduleConceptClassification(ctx, conceptIds);
