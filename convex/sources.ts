...
import { computeCanonicalDedupeKey, extractYouTubeVideoId, generateDedupeKey } from "./sourceUtils";
...
export const recomputeDedupeKeys = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const source of page.page) {
      // Prior collision losers keep their old key by design — never re-plan them.
      if (source.status === "archived" && source.blockedReason === "duplicate") {
        skipped++;
        continue;
      }
