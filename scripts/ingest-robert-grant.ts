...
async function ingestSource(source: {
  ...
  topics?: string[];
  category?: string;
}, type: "pdf" | "article"): Promise<boolean> {
  const key = dedupeKey(type === "pdf" ? "pdf" : "url", source.url);
  
  // Check if already exists
  const existing = await client.query(api.sources.getByDedupeKey, { dedupeKey: key });
  if (existing) {
...
async function addYouTubeFeed(): Promise<void> {
  ...
  const hasGrant = existing.some((f: any) => f.url.includes("UC2MN4AlpbY9NYxuYH-ecoCQ"));
