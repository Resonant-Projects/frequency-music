...
const CONVEX_URL = process.env.CONVEX_URL || 
  "https://righteous-marmot-892.convex.cloud";
...
async function main() {
  ...
  if (args[0] === "--auto") {
    ...
    const extractions = await client.query(api.extractions.listRecent, { limit: 20 });
    
    // Score by: number of claims + number of parameters + has peer_reviewed claims
    const scored = extractions
      .map(e => {
        const peerReviewedClaims = e.claims.filter((c: any) => c.evidenceLevel === "peer_reviewed").length;
        const score = e.claims.length * 2 + e.compositionParameters.length * 3 + peerReviewedClaims * 5;
