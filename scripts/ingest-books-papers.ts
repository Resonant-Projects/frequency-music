...
async function ingestSource(item: {
  title: string;
  author?: string;
  url: string;
  pdf?: string;
  topics?: string[];
  category?: string;
}, type: "book" | "arxiv"): Promise<boolean> {
  const dedupeKey = type === "arxiv" 
...
async function addFeeds(): Promise<void> {
  ...
  const feeds = [
    // Open journals
    { name: "Music Theory Online", url: "https://mtosmt.org/mto.xml", type: "rss" as const },
    // ArXiv categories
    { name: "arXiv: Sound (cs.SD)", url: "https://arxiv.org/rss/cs.SD", type: "rss" as const },
    { name: "arXiv: Audio & Speech (eess.AS)", url: "https://arxiv.org/rss/eess.AS", type: "rss" as const },
    // Journal of Mathematics and Music
    { name: "Journal of Mathematics and Music", url: "https://www.tandfonline.com/feed/rss/tmam20", type: "rss" as const },
  ];
...
async function main() {
  ...
  if (doBooks) {
    ...
    for (const book of sourceData.books.free_pdfs) {
      ...
      if (success) ingested++; else skipped++;
