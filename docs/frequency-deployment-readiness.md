...
The Node supervisor starts a fixed 30-second wall-clock timer immediately before spawning the locally installed tsx inspector. This timer is independent of the inspector event loop, including its startup and synchronous parsing. Deadline, output overflow or interruption kills only that detached capture process group; no CT, Docker or infrastructure operation is involved. It buffers at most 16 MiB of combined child stdout/stderr, discards stderr, and releases stdout only after exit 0 and the expected inspection envelope. It invokes the fixed production origin, accepts no command/URL override, and passes only PATH and the existing inherited admin key to the child. Use an installed supported Node runtime and pinned local dependencies, with no shell tracing or injected Node options. Its deadline supervises the capture subprocess, not operating-system scheduling, supervisor startup or final output-storage availability. A blocked output destination is outside that timing guarantee.
