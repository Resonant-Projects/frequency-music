...
```sh
node scripts/frequency-semantic-capture.mjs
```
...
The Node supervisor starts a fixed 30-second wall-clock timer immediately before spawning the locally installed tsx inspector. This timer is independent of the inspector event loop, including its startup and synchronous parsing. Deadline, output overflow or interruption signals only the live capture process group (never a reaped PID); no CT, Docker or infrastructure operation is involved. It buffers at most 16 MiB of combined child stdout/stderr, discards stderr, and releases stdout only after exit 0 and the expected inspection envelope. It invokes the fixed production origin, accepts no command/URL override, and passes only PATH and the existing inherited admin key to the child. Use an installed supported Node runtime and pinned local dependencies, with no shell tracing or injected Node options. The fixed inspector has no external subprocess contract; this is not a general supervisor for arbitrary commands whose leader exits while leaving detached descendants. Its deadline supervises the capture subprocess, not operating-system scheduling, supervisor startup or final output-storage availability. A blocked output destination is outside that timing guarantee.

The inner inspector retains a shared 20-second abort budget, 4 MiB streamed response per query, 16 MiB aggregate, and at most 100 component records. Queries are sequential with no redirects or retries. Any failed request, malformed/incomplete inventory, duplicate identity or exceeded limit yields a fixed failure JSON and no partial success. These client limits do not bound the server-side metadata scan: the existing system queries can scan all metadata. If its scale makes that unacceptable, retain the gate and use protected recovery records instead of increasing limits automatically.
...
## September 14 coordination checkpoint

PR62 at `c7abbfc611db7eec957181d6b100ff94971dc92d` is an open Mac-owned recovery tooling review. At this refresh its source CI was green, but unresolved findings still concerned first-boot isolation, unsafe-guest stop/cleanup, record-write failure cleanup, host-side deadlines, SysV activation and Linux smoke-test behavior. Green CI does not authorize recovery execution. The guest identity extractor's watchdog is localhost/hash-specific and is not the semantic-capture supervisor above. Mac retains ownership of all recovery scripts and their publication; do not merge or edit those files concurrently from this source thread.
