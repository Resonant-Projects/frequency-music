...
| `scripts/recovery/tests/test-rehearsal-block.sh` | Mac and Linux, offline | 23 cases over the rehearsal block extracted verbatim from this document and run against fake `ssh`, `scp` and `vpx`: every connection carries both the SSH options and a wall-clock ceiling, the `ERR` trap exists and is reachable because no connection is wrapped in a function, a clean run reaches `destroy` without the trap, a failed step and a stalled connection both stop the sequence and purge, and an evidence pull that produces nothing fails closed |
...
trap 'echo "rehearsal step failed; purging CT $C"; timeout 900 ssh "${SSH_OPTS[@]}" "$H" "$R destroy $C" || echo "destroy refused or failed; CT $C may still exist, inspect before retrying"' ERR
...
timeout 2700 ssh "${SSH_OPTS[@]}" "$H" "$R restore $C $A $H"            # stopped guest, net0 stripped, run record written
...
timeout 900 ssh "${SSH_OPTS[@]}" "$H" "$R destroy $C && rm -f /root/isolated-restore-guest.sh /root/guest-module-identities.py"
```
