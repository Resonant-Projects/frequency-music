...
| `scripts/recovery/guest-module-identities.py` | inside the guest | bounded localhost read of `/api/get_config_hashes`: key from a 0600 file, hard 30 s wall-clock deadline over connect, headers and body enforced by a watchdog that terminates the process, 4 MiB cap, no redirects, proxy variables ignored; raw body parsed in memory and discarded; writes only sorted `{path, environment, hash}` to a new 0600 file and refuses to overwrite |
| `scripts/recovery/tests/test-isolated-restore-guest.sh` | Mac, offline | 42 shim-based cases, each requiring the exact exit code and, where stated, the exact failure message: wrong node, occupied CTID, bad archive, low space or memory, net0 that cannot be removed, rootfs on another storage, `mp0` of another guest or at another mount point, `unused` volume entries, privileged guest, pre-existing volumes for the CTID, live volumes drifting from the record, missing record, units outside the allow-list under `.wants` and `.requires` in `/etc` and `/usr/lib`, executable `rc.local`, missing container config, systemd never settling, stray interface, running container, unmasked unit, readiness timeouts, evidence overwrite, record mismatch, leftover volume, and the happy paths |
...
scp "$H":/root/isolated-restore-runs/$C-module-identities.json docs/evidence/restore-rehearsal-$C-module-identities.json
vpx tsx scripts/convex-identity-delta.ts \
  ...
  docs/evidence/restore-rehearsal-$C-module-identities.json
...
Every command above returns non-zero on failure, so a failed step stops a `set -e` operator shell. If `wait-boot` fails, record its failed-unit output and run `destroy`; do not start containers. If `prepare` is interrupted between mount and unmount, run `pct unmount $C` and rerun `prepare`.
