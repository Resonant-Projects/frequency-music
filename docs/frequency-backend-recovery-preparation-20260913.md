...
| --- | --- | --- |
| `scripts/recovery/isolated-restore-guest.sh` | Proxmox node, root | one subcommand per step; every subcommand exits non-zero on any failed check. `restore` repeats preflight, restores a stopped guest, strips the archive's production `net0`, asserts the guest's own `ceph-vm` volumes, and writes a per-run record naming the exact rootfs and data volumes. `prepare`, `start`, `wait-boot`, `verify`, `start-db`, `start-backend`, `read-identities` and `destroy` all refuse unless the live config matches that record; `start` re-asserts the full stopped config immediately before `pct start`; `destroy` checks the record’s CTID and node before stopping a running guest, then checks live configuration drift before purging. Every `pct exec`, `push`, `pull` and `start` runs under a host-side deadline; `pct restore`, `mount` and `unmount` do not, which is why the operator's own step ceilings in the block above matter |
...
| `scripts/recovery/tests/test-isolated-restore-guest.sh` | Mac and Linux, offline | 103 shim-based cases, each requiring the exact exit code and, where stated, the exact failure message: wrong node, occupied CTID, bad archive, low space or memory, net0 that cannot be removed, rootfs on another storage, `mp0` of another guest or at another mount point, `unused` volume entries, privileged guest, pre-existing volumes for the CTID, live volumes drifting from the record, missing record, units outside the allow-list under `.wants` and `.requires` in `/etc` and `/usr/lib`, SysV runlevel links not shadowed by a native unit, executable `rc.local`, a run record that cannot be written after restore, networking or a hookscript attached between `prepare` and `start`, `start` on a guest that is not stopped, symlinked systemd directories and Docker data roots that escape the rootfs, missing container config, systemd never settling, stray interface, running container, unmasked unit, readiness timeouts, guest calls without a host-side deadline, evidence overwrite, record mismatch, an unsafe running guest at `destroy` (stopped, then refused), a guest that does not stop, leftover volume, and the happy paths |
| `scripts/recovery/tests/smoke-guest-module-identities.sh` with `fake-backend.py` | Mac and Linux, offline | success without configuration leakage, overflow, HTTP error, malformed identity, redirect refusal, loose key permissions, rerun refusal, and three deadline reproductions (one incomplete chunk held open, no headers ever, and a drip that never trips the per-socket timeout) each asserted to finish within 5 s under a 2 s deadline with a sanitized failure and no output file |
| `scripts/recovery/tests/test-rehearsal-block.sh` | Mac and Linux, offline | 27 cases over the rehearsal block extracted verbatim from this document and run against fake `ssh`, `scp` and `vpx`: every connection carries both the SSH options and a wall-clock ceiling, the `ERR` trap exists and is reachable because no connection is wrapped in a function, a clean run reaches `destroy` without the trap, a failed step and a stalled connection both stop the sequence and purge, and an evidence pull that produces nothing fails closed |
...
What `prepare` does on the mounted, stopped rootfs before first boot: asserts the run record and that `rootfs` and `mp0` are `ceph-vm:vm-<ctid>-disk-N` with `mp0` at `/srv/app-data`, the guest is unprivileged, `onboot 0`, no `net*`, `mp1+`, `unused*`, `dev*`, `hookscript` or raw `lxc.` keys (the same assertions run inside `restore` before the record is written, and `preflight` refuses if any `vm-<ctid>-` volume already exists on `ceph-vm`); masks `app-compose.service`, all `lab-*` timers and services, `cron.service`, apt timers, unattended upgrades and postfix by replacing them with `/dev/null` symlinks and deleting their `.wants` links; removes `/etc/rc.local`, enumerates every remaining `.wants` and `.requires` link under `/etc/systemd/system`, `/usr/lib/systemd/system` and `/lib/systemd/system` plus every `S*` link under `/etc/rc[2-5].d` and `/etc/rcS.d` that no native unit shadows (systemd-sysv-generator would otherwise turn it into a boot dependency), and fails unless each is on an explicit allow-list (Docker, containerd, ssh, getty, networking for `lo`, the named systemd units observed on CT113 and the harmless local maintenance timers), printing the surviving list as evidence; rewrites every Docker `hostconfig.json` restart policy to `no` (reading `data-root` from `daemon.json` if present) and fails if no container config was found; removes the 1Password service-account env and registry auth files; writes a marker file. Docker stays enabled so the operator can start exactly two containers. `start` refuses unless the run record matches, the guest is stopped and the full stopped config passes the same assertions, then runs `pct start` and exits non-zero unless systemd reaches `running` or `degraded` within its bounded attempts, printing failed units otherwise; every `pct exec`, `push`, `pull` and `start` in the helper runs under a host-side `timeout`, so a hung guest cannot stall the helper or keep the caller's cleanup trap from running. `verify` runs before any container starts and fails on any network entry in the host config, any guest interface that is neither `lo`, a bridge, nor bridge-enslaved (dockerd creates `docker0` and compose bridges at daemon start with no network reach), any masked unit not reported `masked`, any running container, or missing `curl`, `timeout` or `python3`.
...
RESTORE_UNIT="isolated-restore-$C-$(date -u +%Y%m%dT%H%M%SZ).service"
...
# The keepalives above only catch a transport that stops answering. A host that
# stays responsive while a step wedges -- pct restore, pct mount and pct unmount
# carry no deadline of their own -- would still block this shell forever, so the
# ERR trap would never run and CT $C would outlive the run. Each step therefore
# also carries a wall-clock ceiling. These are stall ceilings, not expected
# durations: only restore is long, because it unpacks a 17 GiB archive.
# On any failure after restore, purge the guest so no copy of production credentials or volumes outlives the run.
trap 'echo "rehearsal step failed"' ERR
trap 'exit 129' HUP
trap 'exit 130' INT
trap 'exit 143' TERM
trap 'rc=$?; trap - EXIT HUP INT TERM; echo "rehearsal step failed; purging CT $C"; timeout 900 ssh "${SSH_OPTS[@]}" "$H" "systemctl stop \"$RESTORE_UNIT*\" && $R destroy $C" || echo "destroy refused or failed; CT $C may still exist, inspect before retrying"; exit "$rc"' EXIT
timeout 300 scp "${SSH_OPTS[@]}" scripts/recovery/isolated-restore-guest.sh scripts/recovery/guest-module-identities.py "$H":/root/
timeout 120 ssh "${SSH_OPTS[@]}" "$H" "$R preflight $C $A $H"           # read-only
# ---- first mutation begins here ----
timeout 2700 ssh "${SSH_OPTS[@]}" "$H" "systemd-run --unit=$RESTORE_UNIT --service-type=exec --wait --pipe --property=RuntimeMaxSec=2400 --property=TimeoutStopSec=30 --property=KillMode=control-group $R restore $C $A $H"            # stopped guest, net0 stripped, run record written
timeout 900 ssh "${SSH_OPTS[@]}" "$H" "$R prepare $C"                   # offline suppression on the mounted rootfs
timeout 600 ssh "${SSH_OPTS[@]}" "$H" "$R start $C"                     # re-asserts the stopped config, pct start, non-zero if systemd never settles
timeout 300 ssh "${SSH_OPTS[@]}" "$H" "$R verify $C"                    # must pass before any docker start
timeout 600 ssh "${SSH_OPTS[@]}" "$H" "$R start-db $C"                  # sizes and counts before the backend runs
timeout 600 ssh "${SSH_OPTS[@]}" "$H" "$R start-backend $C"             # prints /version
IDENTITY_OUTPUT=$(timeout 600 ssh "${SSH_OPTS[@]}" "$H" "$R read-identities $C /root/guest-module-identities.py")
printf '%s\n' "$IDENTITY_OUTPUT"
REMOTE_IDENTITIES=$(printf '%s\n' "$IDENTITY_OUTPUT" | awk '/^identities file: / {print $3}')
[[ "$REMOTE_IDENTITIES" =~ ^/root/isolated-restore-runs/$C-[0-9]{8}T[0-9]{6}Z-module-identities\.json$ ]]
IDENTITIES="docs/evidence/${REMOTE_IDENTITIES##*/}"
[ ! -e "$IDENTITIES" ]   # refuse stale local evidence from this exact run
timeout 300 scp "${SSH_OPTS[@]}" "$H:$REMOTE_IDENTITIES" docs/evidence/
[ -f "$IDENTITIES" ]   # fails the step if the pull produced nothing
...
timeout 900 ssh "${SSH_OPTS[@]}" "$H" "systemctl stop \"$RESTORE_UNIT*\" && $R destroy $C && rm -f /root/isolated-restore-guest.sh /root/guest-module-identities.py"
trap - EXIT HUP INT TERM ERR
```

The restore runs in a uniquely named systemd service on the Proxmox host. Its 2400-second runtime limit and 30-second stop grace apply to the entire control group, independently of the local SSH connection. Both cleanup paths synchronously stop that unit before invoking `destroy`; a failed stop prevents purge and reports the need for inspection. The quoted unit-name glob also succeeds when systemd has already unloaded the completed unit; it is scoped to this unique run. If restore is killed before the run record is written, `destroy` refuses the unrecorded guest: inspect the stopped operation's partial config and volumes before retrying. The service prevents a surviving restore worker from creating the guest after cleanup has already returned.

Run the block in Bash, from this repository, on a Mac with GNU `timeout` (`brew install coreutils`; use `gtimeout` if only that name exists). Every SSH and SCP invocation, including the cleanup trap, uses a 15-second connection timeout, a single connection attempt, batch mode (no interactive authentication prompts), and server-alive probes that disconnect an unresponsive transport after approximately 45 seconds. Those settings detect connection loss. Because they do not bound a responsive host running a wedged command, and because `pct restore`, `pct mount` and `pct unmount` carry no deadline inside the helper, each step also has a wall-clock ceiling; without one a wedged step blocks the operator's shell, the `ERR` trap never runs, and the guest outlives the run. Guest operations retain the helper's own host-side deadlines underneath. The commands are written inline rather than wrapped in shell functions on purpose: `set -e` runs the `ERR` trap for a failing top-level command, but a failure inside a function is not covered unless the block also sets `-E`. If the host is unreachable, cleanup cannot be guaranteed: the trap reports failure and the operator must inspect and remove the recorded guest when access is restored.

Every command above returns non-zero on failure and the block runs under `set -e` with an `ERR` trap that runs `destroy`, so a failed step stops the sequence and attempts to purge the guest. `restore` itself purges a guest it just created if isolation cannot be established or the run record cannot be written, so no stopped copy with production network settings or credentials is left behind by a partial run; if that purge itself fails, `restore` says so and exits non-zero. `destroy` refuses a guest without a matching run record, in which case the operator inspects before retrying. `start` re-asserts the full stopped config immediately before `pct start`, so nothing attached between `prepare` and the first boot can run with connectivity. `start-db`, `start-backend` and `read-identities` each re-run the live isolation gate (host config with no `net`, `mp1+`, `unused`, device or `hookscript` keys; prepare marker; guest interfaces; masked units; only the containers expected at that step running), so skipping `verify` or attaching anything afterwards fails closed. `destroy` first verifies the record’s CTID and node, then stops a running guest, then re-asserts the full config immediately before purge; an unsafe config leaves the guest stopped and unpurged for inspection rather than running. If `start` fails, its failed-unit output is already printed and the trap runs `destroy`; do not start containers. If `prepare` is interrupted between mount and unmount, run `pct unmount $C` and rerun `prepare`.
...
Revision 2 received an independent adversarial review (14 defects, all fixed in revision 3). Revision 3 received coordinator review: the deadline was per-socket rather than wall-clock, the equality claim was overstated, volume ownership was not asserted, cleanup keyed on reusable hostname and tag, and readiness relied on operator inspection. Revision 4 addresses each with a reproduction or test: three deadline reproductions in the smoke test (plus four further adversarial drips run by the spec reviewer, all bounded at the deadline), 84 helper cases with exact exit codes and messages, the run record with exact volumes, and the corrected interpretation above. Revision 4's diff then received a standards review and a spec review; their findings (ownership tests that passed for the wrong reason, non-exact exit-code assertions, missing `unused` and orphan-volume checks, `.requires` and vendor enablement directories, a broad systemd glob, a fixed smoke-test port, older-Python timeout classification) are all addressed in the published revision. After publication, Macroscope, Codex and CodeRabbit reviews on the pull request raised: loopback-only extractor origin, archived hookscript, destroy re-assertion, symlinked systemd directories and Docker data roots escaping the rootfs, a guest left behind after a failed restore or a failed record write, the first boot not gated on the current stopped config, a running unsafe guest left running when `destroy` refused, guest calls without host-side deadlines, SysV runlevel links, the smoke test's `stat` invocation failing on GNU coreutils, and failure cases that did not assert the absence of an output file. Each is fixed with a test in the shim or smoke suite, and both suites now pass on Linux (GNU coreutils 9.7, Python 3.12 in a container) as well as on the Mac. A second independent adversarial review of the published head found nine further defects, all fixed here with a regression test that was mutation-checked against the fix: `prepare` wrote its marker through `>`, so an archived symlink at that fixed path could truncate a file on the Proxmox host as root; `find` does not descend into a symlinked `.wants` or `.requires` directory although systemd follows one, so units enabled inside it were invisible to the allow-list, and such a directory is now refused outright; piping `pct config` into `grep -q` could leave the writer with SIGPIPE, which `pipefail` turns into 141 so the network gate's `&& die` never ran, and the config is now read once into a variable, which also closes the window between what were seven separate reads; `destroy` checked the run record before stopping a running guest, so a drifted record left it running; `restore` reported a guest as purged without confirming the config and volumes were gone; the tag check matched a substring, so `not-disposable-restore-x` passed; and `strip_networking` and `write_record` relied on `set -e`, which bash disables inside the `if ! ( ... )` purge guard. Three mutants that the previous suite let through are now caught: an unwrapped `pct start`, a suppressed purge failure, and a SysV scan narrowed to one runlevel directory. Checks that pass now: 16 provenance unit tests including the identity delta; repository format, lint and all typechecks; `vp run verify` with 666 tests across the root, web and agent packages; `bash -n` and Python byte-compilation; both offline suites via `vp run test:recovery`. The recovery suites are not part of `verify` because the CI runners do not provision Python. Proxmox option names, the mount path and the `pvesm` column positions were confirmed against `pct help` and live output on prox5 (pve-manager 9.2.11). Claims that rest on the homelab-infra repository are cited by file and are not verifiable from this repository alone. 
...
Host-side guest timeout wrappers escalate to SIGKILL five seconds after their TERM deadline. Admin-key creation first unlinks the old path and uses shell noclobber creation, preventing archived symlinks or hard links from receiving the key.

A failed `pct restore` attempts cleanup only when its partial guest passes the stopped-state, isolation, identity and volume checks and a run record can be written. Unverifiable partial configuration or orphan volumes are left for explicit inspection.
