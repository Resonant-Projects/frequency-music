...
| `scripts/recovery/tests/test-rehearsal-block.sh` | Mac and Linux, offline | 18 cases over the rehearsal block extracted verbatim from this document and run against fake `ssh`, `scp` and `vpx`: every connection carries both the SSH options and a wall-clock ceiling, the `ERR` trap exists and is reachable because no connection is wrapped in a function, a clean run reaches `destroy` without the trap, a failed step and a stalled connection both stop the sequence and purge, and an evidence pull that produces nothing fails closed |
