...
| `listDraftableCorrespondences` | `/agent-tools/listDraftableCorrespondences` | `internal.agentDrafts:listDraftableCorrespondences` | List bounded, hydrated correspondence targets that have neither an existing hypothesis nor a pending hypothesis draft. | Returns evidenced targets first, then conjectured targets ranked by similarity times novelty, with exact evidence-claim provenance. |
