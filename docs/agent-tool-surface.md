...
This document defines the read-only Convex surface exposed to external LangGraph/LangChain agents.

The first phase is deliberately narrow: agents can read research state, but they cannot create or mutate project data. Write tools need a separate human-in-the-loop design before they are exposed.
...
Do not add write tools in this phase. Candidate write tools such as \`createHypothesisDraft\` and \`markFailure\` belong in the later LangGraph plan after approval flows are designed.
