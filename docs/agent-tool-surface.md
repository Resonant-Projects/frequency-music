...
| `createAgentReviewDraft` | `/agent-tools/createAgentReviewDraft` | `agentDrafts:createFromAgentRun` | Persist a `pending_review` draft carrying an optional structured `payload` (hypothesis or recipe). | `whyThisMatters` is enforced at draft-creation time; payload-less drafts are acknowledge-only and can never be promoted. |
