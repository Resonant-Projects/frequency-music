...
_Every score has two parts: the structure it specifies, and the act that turns that structure into sound. Recent extractions keep showing the same hidden layer — not the rule system, not the waveform, but the realization function between them._
