...
The explanation of Classifier-Free Guidance (CFG)—pushing the generation _away_ from an unconditional prediction and _toward_ the conditioned prompt—is handled perfectly. It gives the reader a solid mental model of how the "Prompt Weight" slider actually works under the hood.
