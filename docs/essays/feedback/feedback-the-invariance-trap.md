...
The core argument is excellent. You take a highly specific paper (ID-LoRA and negative RoPE embeddings) and use it to explain _why_ it's so hard to train a model to change the melody (the action) without changing the singer (the identity).
