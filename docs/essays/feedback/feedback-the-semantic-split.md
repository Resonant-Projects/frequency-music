...
The core argument is that text embeddings (semantics) and audio embeddings (acoustics) inhabit fundamentally different topological spaces, and forcing them together creates a "semantic split" where the model must choose between what a sound *is* and what a sound *means*. 
...
> "The model doesn't know what a guitar sounds like; it knows what the *word* 'guitar' is statistically adjacent to in the training data."
