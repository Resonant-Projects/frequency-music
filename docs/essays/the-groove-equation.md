...
What's mathematically interesting here is that the analog processing chain introduces *micro-timing perturbations* — tiny, non-random deviations from the quantization grid that are characteristic of specific hardware. An MPC's timing has a different statistical signature than an SP-1200's. These perturbations create *structured micro-deviations* — not the same phenomenon as the polska's non-isochronous beats (which are macroscopic, culturally codified metric structures), but operating at a different scale with a parallel principle: the groove feels human not because of random variation, but because of *patterned* deviation. The analogy has limits — polska changes the actual duration of beats, while MPC jitter sits around an isochronous grid — but both reveal that rhythmic life comes from systematic departure from uniformity.
...
Swing applies asymmetric time division at the *subdivision* level — the beat-level grid remains isochronous, but pairs of sixteenths within each beat are made unequal. A swing of 25% (MPC sense — the off-beat note shifts 25% later, so the pair splits 62.5%/37.5%) creates a ratio close to 5:3. Polska asymmetry operates at the *beat* level — the beats themselves have unequal durations (roughly 3:2:2). These are different metric levels, with different biomechanical implications: a swung techno track still has a steady quarter-note pulse for the body to lock onto, while a polska forces the dancer to internalize an uneven beat pattern. The mathematical *ratios* are similar (~5:3 vs ~3:2), but the structural level at which they operate — and thus their felt effect — is fundamentally different.
