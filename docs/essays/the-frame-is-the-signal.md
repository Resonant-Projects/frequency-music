# The Frame Is the Signal: When Context Overpowers Content
...
## The Problem of Eavesdropping Models

A striking paper on automatic depression detection reveals something unsettling: machine learning models trained on clinical interview recordings learn to detect depression not from what the patient says, but from what the interviewer asks. The models exploit fixed interviewer prompts and positional artifacts — where in the conversation a segment appears — rather than anything about the respondent's actual language. The bias is cross-dataset and architecture-agnostic. It's not a quirk; it's the dominant learned strategy.

This is typically framed as a failure mode. But from a musical perspective, it's a revelation about how signals work.
...
- Generic convention (post-climax plateau: handoff)

The decision can't be read from the acoustic signal alone. It requires the frame.

## Compositional Implications

If the frame is the signal — or at least a signal as strong as the content — this has practical consequences for composition:

**1. You can compose the frame independently of the content.**

Film composers know this intuitively. A simple sustained chord becomes terrifying with the right visual framing, becomes transcendent with another. But the principle extends within pure music: writing an introduction is composing a frame for whatever follows. A slow tempo markup after a scherzo reframes the scherzo retroactively as manic rather than playful.
...
A distorted guitar chord means one thing in metal, another in shoegaze, another in a Ligeti piece. The genre frame — established before a single note plays, through album art, venue, social context — is arguably the strongest contextual force in how music is perceived. Composers who work across genres are, in this framing, people who compose at the level of the frame itself.

## The Self-Decomposing Frame

Essays #73 and #74 argued that audio is self-decomposable: signals carry enough internal information to separate their own components. But the depression detection paper shows the dark side — some of that "internal information" is structural artifact rather than genuine content.

Perhaps the same is true of music. When we analyze a piece and find "tension" and "resolution," are we detecting properties of the sound, or are we detecting our own learned frames? The overtone series provides real acoustic grounding for consonance. But the experience of tension in a minor second versus a major seventh — acoustically similar in roughness — is almost entirely frame-dependent (voice leading expectations, registral conventions, stylistic context).

This doesn't make musical analysis invalid. It makes it richer. The frame is real information. It's just not where we usually look.

## The Takeaway

The ML researchers framed positional bias as a bug to be corrected. But for music, the insight is more profound: **the most powerful signals in structured temporal media are often positional, not content-based.** Where something happens matters as much as what happens. The frame shapes the signal at least as strongly as the signal fills the frame.
