...
As argued in "The Locking In," this isn't metaphor. The cochlea literally operates as a bank of coupled nonlinear oscillators, and mode-locking is the physical mechanism of consonance perception. The devil's staircase is the mathematical object that *generates* the scale from the continuum. Discreteness isn't imposed by convention — it emerges from the physics of coupled oscillators.
...
The cochlea performs something remarkably like analog-to-digital conversion, but with a twist that no engineer would design.

The basilar membrane is a continuous resonator bank — each position along its 35mm length responds to a different frequency, with the mapping approximately logarithmic. This is the continuous, analog front end. But the hair cells that transduce mechanical vibration into neural impulses fire discretely — either they fire or they don't, at specific moments, creating a pulse train. The auditory nerve carries not a continuous signal but a stream of discrete spikes.

The neural representation of pitch is a hybrid. For frequencies below ~4 kHz, the timing of spike volleys encodes the period of the waveform — a form of temporal coding that preserves continuous phase information. For higher frequencies, only the place (which hair cells fire most) carries pitch information — a coarser, more categorical encoding. The transition between these regimes around 4 kHz is not coincidental: it's approximately where melodic pitch perception degrades, where the sense of "noteness" weakens. The ear literally becomes more discrete-like in its encoding at frequencies where music becomes less useful.
