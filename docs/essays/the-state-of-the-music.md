...
**State resets.** A dramatic silence, a sudden key change, or a textural rupture can be understood as a *state reset* — zeroing out some dimensions of *x* and forcing the listener to rebuild context. The effectiveness of such gestures depends on *which* dimensions get reset. A silence after a long development section resets temporal expectations while preserving tonal memory; a sudden modulation resets tonal memory while preserving rhythmic momentum.
