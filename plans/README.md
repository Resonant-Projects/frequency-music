...
| 014 | Security hardening: bump `ws` (GHSA), constant-time bypass compare | P2 | S | — | DONE (root + agent) 2026-07-18 — root ws@8.21.0 and `agent/` ws@8.21.1, each via its own override; no `ws` advisory in either. Constant-time compare landed. **Residual — `web/` only:** it is a separate install (no root `workspaces` field) and still resolves `ws@8.18.0`. Deliberately deferred: `web/` also carries `ws@7.5.10` (metro/react-native-dev-middleware/jayson) which is OUTSIDE the advisory range `>=8.0.0 <8.21.0`, so a name-level override breaks it (verified: 7.5.10 vanishes from the lockfile), and `web/package.json` floats `vite`/`vitest` at `@latest` (finding DEP-01) so any re-resolve churns the toolchain. Needs path-scoped overrides — carded. Exposure is low: the web app is client-rendered. `lint:check` gate pre-existing-broken on main at the time (waived; cleared by 016). |
