...
import {
  SECTORS, PIPELINE_RINGS, R, COLORS,
  type SectorDef, arcMidpoint, dotWorldPos,
} from './zodiac-data'
import {
  makeSectorFillMat, makeArcMat, makeSourceMat, makeHubMat,
  makeHubRingMat, makeHubVioletMat, makePipelineRingMat,
  makeOuterRingMat, makeTickMat, makeBackgroundDotsMat,
...
function arcPoints(r: number, startAngle: number, endAngle: number, segments = 64, z = 0): THREE.Vector3[] {
...
function circleLine(r: number, mat: THREE.LineBasicMaterial | THREE.LineDashedMaterial, z = 0): THREE.Line {
...
export function buildSectorGroup(sector: SectorDef, active = false): THREE.Group {
  ...
  const spokePts = [polar3(R.sectorInner, sector.startAngle), polar3(R.sectorOuter, sector.startAngle)]
...
export function buildSourceNodes(scene: THREE.Scene): Array<{ mesh: THREE.Mesh; sectorId: string }> {
...
export function getLabelPositions(): Array<{ sector: SectorDef; x: number; y: number; z: number; rotZ: number }> {
