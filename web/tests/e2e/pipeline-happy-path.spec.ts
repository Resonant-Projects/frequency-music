...
test.describe("pipeline happy path", () => {
  ...
  test("creates and advances core records across routes", async ({ page }) => {
    ...
    await test.step("ingest URL source", async () => {
      ...
      await page.locator("#url-input-raw-text").fill(`E2E ingest payload for ${runId}`);
    ...
    await test.step("triage source in display queue", async () => {
      ...
      let sourceRow = page.getByTestId("display-row").filter({ hasText: sourceTitle }).first();

      const hasNewSource = await sourceRow.isVisible({ timeout: 5_000 }).catch(() => false);
    ...
    await test.step("create hypothesis", async () => {
      ...
      await page
        ...
        .fill(`If ${runId} keeps a stable anchor, perceived cohesion should increase.`);
    ...
    await test.step("create recipe from hypothesis", async () => {
      ...
      await expect
        .poll(
          async () =>
            page.locator("#recipe-hypothesis option").filter({ hasText: hypothesisTitle }).count(),
          { timeout: 30_000 },
        )
        .toBeGreaterThan(0);
    ...
    await test.step("create composition and set rendered status", async () => {
      ...
      await expect
        .poll(
          async () =>
            page.locator("#composition-recipe option").filter({ hasText: recipeTitle }).count(),
          { timeout: 30_000 },
        )
        .toBeGreaterThan(0);
      await page.locator("#composition-title").fill(compositionTitle);
      await page.locator("#composition-recipe").selectOption({ label: recipeTitle });
    ...
    await test.step("log listening feedback", async () => {
      ...
      await expect
        ...
        .toBeGreaterThan(0);
      await page.locator("#feedback-composition").selectOption({ label: compositionTitle });
    ...
    await test.step("create and toggle feed in ingest", async () => {
      ...
      const feedRow = page.getByTestId("ingest-feed-row").filter({ hasText: feedName }).first();
    ...
    await test.step("verify weekly turns route renders", async () => {
      ...
      await expect(page.getByRole("heading", { name: "Weekly Turns" })).toBeVisible();
