...
test.describe("tab ownership", () => {
  test("ingest owns feed controls and does not expose extraction actions", async ({
    page,
  }) => {
    await page.goto("/ingest");

    await expect(
      page.getByRole("heading", { name: "Feed Intake" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: "Poll Feeds Now" }),
    ).toBeVisible();
  ...
  test("admin excludes feed operations and emphasizes override-only controls", async ({
    page,
  }) => {
    await page.goto("/admin");

    await expect(
      page.getByRole("heading", { name: "Intake Controls Moved" }),
    ).toHaveCount(0);
    await expect(
    ...
    await expect(page.getByRole("heading", { name: "Feed List" })).toHaveCount(
      0,
    );
  });
  ...
  test("display owns source extraction action", async ({ page }) => {
    ...
    await expect
      .poll(
        async () =>
          page.getByRole("button", { name: "Run Extraction" }).count(),
        {
          timeout: 30_000,
        },
      )
