...
test.describe("phase two surfaces", () => {
  test("phase two routes and new decision-support sections render", async ({
    page,
  }) => {
    ...
    await test.step("verify display page shows editorial signals", async () => {
      ...
      await expect(
        page.getByRole("heading", { name: "Editorial Signals" }),
      ).toBeVisible();
    });
    ...
    await test.step("compositions page renders phase two entry point", async () => {
      ...
      await expect(
        page.getByRole("heading", { name: "Compositions" }),
      ).toBeVisible();
