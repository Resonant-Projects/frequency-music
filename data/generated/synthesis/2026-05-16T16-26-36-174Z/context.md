...
Generated at: 2026-05-16T16:26:36.174Z
...
- Prior topic tokens tracked: 239
- Prior variable phrases tracked: 12
...
- nonstationary signals: 1
- acoustic feature fusion: 1
- temporal sequence modeling: 1
- legendre memory unit (lmu): 1
- pitch contour analysis: 1
- biological acoustics: 1
- domain generalization in audio: 1
- real-time audio processing: 1
- convolutional neural networks for audio: 1
...
### S1 — Building dialogue in electronic music with Kontakt, Monark, and David Mayer
...
- Scores: base=29.00, normalized=0.537, intraNovelty=1.000, crossRunNovelty=1.000, topicalBalance=1.000, combined=0.807
...
### S2 — Asymmetric Phase Coding Audio Watermarking
...
- Scores: base=34.00, normalized=0.659, intraNovelty=1.000, crossRunNovelty=0.925, topicalBalance=0.978, combined=0.803
...
### S4 — Predicting Upcoming Stuttering Events from Three-Second Audio: Stratified Evaluation Reveals Severity-Selective Precursors, and the Model Deploys Fully On-Device
...
- Scores: base=17.00, normalized=0.244, intraNovelty=1.000, crossRunNovelty=0.975, topicalBalance=0.970, combined=0.673
...
### S5 — Omni2Sound: Towards Unified Video-Text-to-Audio Generation
...
- Scores: base=15.00, normalized=0.195, intraNovelty=1.000, crossRunNovelty=1.000, topicalBalance=0.993, combined=0.667
- Domain relevance: 0.400
- Reuse penalties: source=0.00, topic=0.00
...
### S6 — LMU-Based Sequential Learning and Posterior Ensemble Fusion for Cross-Domain Infant Cry Classification

- Source ID: jx72jqrcp519f3tmk8hpfd7wfn86qwpp
- Extraction ID: j9735j1x9c8dxr97dax746vccd86q4tz
- Type: rss
- Status: extracted
- URL: https://arxiv.org/abs/2603.02245
- Scores: base=17.00, normalized=0.244, intraNovelty=1.000, crossRunNovelty=0.975, topicalBalance=0.993, combined=0.662
- Domain relevance: 0.300
- Reuse penalties: source=0.00, topic=0.08
- Topics: fundamental frequency (f0), mel-frequency cepstral coefficients (mfccs), short-time fourier transform (stft), nonstationary signals, acoustic feature fusion, temporal sequence modeling, legendre memory unit (lmu), pitch contour analysis, biological acoustics, domain generalization in audio, real-time audio processing, convolutional neural networks for audio

Summary:
This paper proposes a machine learning framework for classifying the causes of infant cries using acoustic features including MFCCs, STFT, and fundamental frequency (F0) contours. The system uses a multi-branch CNN encoder combined with a Legendre Memory Unit (LMU) for temporal modeling, which is presented as more parameter-efficient than LSTMs. A calibrated posterior ensemble fusion method with entropy-gated weighting is introduced to handle cross-domain generalization across different infant datasets. The work is evaluated on Baby2020 and Baby Crying datasets, demonstrating improved macro-F1 scores. The framing is primarily healthcare/monitoring, but the acoustic feature pipeline is directly relevant to general nonstationary signal analysis.

Claims:
- [1] [preprint] Infant cry signals are short and nonstationary, which makes them challenging to classify acoustically.
- [2] [preprint] Fundamental frequency (F0) contours, when combined with MFCCs and STFT features, improve acoustic classification of nonstationary biological signals.
- [3] [preprint] The Legendre Memory Unit (LMU) provides stable temporal sequence modeling with substantially fewer recurrent parameters compared to LSTMs, making it suitable for real-time on-device acoustic analysis.
- [4] [preprint] Acoustic signals from different individuals (or datasets) exhibit strong domain shifts, suggesting that F0 and spectral features are highly speaker/source-dependent even within the same sound class.

Composition Parameters:
- measurement: Mel-Frequency Cepstral Coefficients (MFCCs)
- measurement: Short-Time Fourier Transform (STFT) features
- frequency: Fundamental Frequency (F0) contours

Open Questions:
- Can F0 contour tracking methods developed for infant cry classification be repurposed for microtonal or expressive pitch analysis in musical performance?
- How does the LMU's approach to temporal modeling compare to recurrent architectures used in music generation or beat tracking tasks?
- Do the 'domain shifts' observed across infant cry datasets have structural analogues in cross-instrument or cross-performer timbre variation in music?
- Could entropy-gated ensemble fusion be applied to combine models trained on different tuning systems or musical traditions for cross-cultural music analysis?
- What is the typical F0 range of infant cries, and does it overlap meaningfully with pitched musical instruments or vocal registers?
