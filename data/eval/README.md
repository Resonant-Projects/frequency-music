...
| **Golden** | `extractions-golden.jsonl`, `hypotheses-golden.jsonl`, `weekly-briefs-golden.jsonl` | **human curation** of candidates | `scripts/langsmith/upload-datasets.ts` |
...
2. **Human curates** candidates into the plural `*-golden.jsonl` files —
   target ≥15 extraction, ≥15 hypothesis, ≥6 brief rows.
3. `bun scripts/langsmith/upload-datasets.ts` pushes golden files into LangSmith
   as `resonant-{extractions,hypotheses,weekly-briefs}-golden`.
