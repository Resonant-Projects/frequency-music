...
describe("dashboard editorial signals", () => {
  test("scores a bounded mention-index candidate through indexed graph links", async () => {
    ...
    await t.run(async (ctx) => {
      ...
      const hypothesisId = await ctx.db.insert("hypotheses", {
        title: "Cymatics hypothesis",
        question: "Do cymatic ratios produce stable figures?",
        hypothesis: "Stable ratios produce stable figures",
        rationaleMd: "Harness fixture",
        sourceIds: [],
        concepts: ["cymatics"],
        status: "active",
    ...
    const result = await t.query(api.dashboard.editorialSignals, { limit: 1 });
    expect(result.concepts).toEqual([
      expect.objectContaining({
        conceptName: "cymatics",
        hypothesisCount: 1,
        linkedRecipes: 1,
        linkedCompositions: 1,
        yieldBand: "high",
      }),
    ]);
  });
