...
describe("research pipeline smoke planning", () => {
  test("requires explicit opt-in by default", () => {
    ...
    expect(
      shouldRunResearchPipelineSmoke({ RUN_RESEARCH_PIPELINE_SMOKE: "false" }),
    ).toBe(false);
  });
  ...
  test("prefers CONVEX_SITE_URL and falls back to Convex URL names", () => {
    expect(
      getConvexSiteUrlFromEnv({ CONVEX_SITE_URL: "https://site.example" }),
    ).toBe("https://site.example");
    expect(
      getConvexSiteUrlFromEnv({ CONVEX_URL: "https://convex.example" }),
    ).toBe("https://convex.example");
    expect(
      getConvexSiteUrlFromEnv({
