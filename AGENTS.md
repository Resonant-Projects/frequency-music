...
- Operator evidence exception: `scripts/frequency-queue-evidence.ts`, `scripts/convex-provenance-verify.ts`, and `scripts/convex-module-identities.ts` use inherited `CONVEX_SELF_HOSTED_ADMIN_KEY` only, without `.env` or Varlock loading. See `docs/frequency-worker-handoff.md` for their read-only scope and deployment prerequisites.
